...
void Config_Postprocess() {
  ...
  void Config_StoreSettings()  {
    ...
    uint8_t leveling_is_on = false;
      ...
      if (planner.abl_enabled) leveling_is_on = 2;
    #elif ENABLED(AUTO_BED_LEVELING_LINEAR)
      if (planner.abl_enabled) leveling_is_on = 3;
    #elif ENABLED(AUTO_BED_LEVELING_BILINEAR)
      if (planner.abl_enabled) leveling_is_on = 4;
    #endif
...
// bob     const bool leveling_is_on = TEST(mbl.status, MBL_STATUS_HAS_MESH_BIT);
...
// bob      const bool leveling_is_on = false;
      dummy = 0.0f;
  ...
  void Config_RetrieveSettings() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      //
        ...
        SERIAL_PROTOCOLLNPAIR("leveling_is_on: ", leveling_is_on);
         #if ENABLED(AUTO_BED_LEVELING_3POINT)
          if (leveling_is_on == 2 && !planner.bed_level_matrix.is_blank()) {
            ABL_grid_valid = true;
            SERIAL_PROTOCOLLNPGM("valid grid restored");
          }
          else {
            ABL_grid_valid = false;
            SERIAL_PROTOCOLLNPGM("invalid grid");
          }  
        #elif ENABLED(AUTO_BED_LEVELING_LINEAR)
          if (leveling_is_on == 3 && !planner.bed_level_matrix.is_blank()) {  
            ABL_grid_valid = true;
            SERIAL_PROTOCOLLNPGM("valid grid restored");
          }
          else {
      ...
      EEPROM_READ(grid_max_y);                       // 1 byte
        ...
        if (grid_max_x == ABL_GRID_MAX_POINTS_X && grid_max_y == ABL_GRID_MAX_POINTS_Y) {
          ...
          SERIAL_PROTOCOLLNPAIR("leveling_is_on: ", leveling_is_on);
          if (leveling_is_on == 4  && bed_level_grid[0][0] != dummy) {
            ABL_grid_valid = true;
            SERIAL_PROTOCOLLNPGM("valid grid restored");
          }
          else ABL_grid_valid = false;
        }
